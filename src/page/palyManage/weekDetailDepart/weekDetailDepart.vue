  ...
  <div>
    ...
    <div class="wrapper">
      ...
      <div class="my-comment">
        <h3 class='comment-title'>二、部门负责人总结</h3>
        <div class="comment-content">
          {{deptHeaderComment}}
        </div>
      </div>
      <div class="my-comment" v-if='weeklyDeptLevel=="LEVEL_TWO"'>
        <h3 class='comment-title'>三、部门负责人总结</h3>
        <div class="comment-content">
          {{secondDeptHeaderComment}}
        </div>
      </div>
      <div class="my-comment">
        <h3 class='comment-title'>{{taskTwo}}、总经理点评</h3>
          <textarea name="name" rows="8" cols="80" v-model='commentRate' style='width:1115px;height:98px;padding:15px;margin:20px;'></textarea>
      </div>
      <div class="button-close" @click='save'>
        确定
      </div>
      <div class="my-comment" style="background: rgba(217,52,55,0.02);border-bottom: none;">
        <h3 class='comment-title' style="background: rgba(217,52,55,0.02);">附：部门员工周报</h3>
      </div>
      <div class="get-reward">
        <el-table
          :data="result"
...
<script type="text/ecmascript-6">
  // 注意标签和引入驼峰，可以有区别
  import Vue from 'vue'
  import { getByDepartWeekly,publishDraftTasks,listAllUsers,listReportingUsers,departmentList,getProjectList,listTaskExecutors,userTaskReply,listByExecutor,manageWeeklyPlan,departReviewWeekly,manageWeeklyComment, manageCommentDepart,dragTask} from 'service/getData'
...
//        console.log(this.checkedValue)
      ...
      save(){
        ...
        if(this.commentRate==''||this.commentRate==null){
          ...
          manageCommentDepart(params).then((res) => {
            if(res.code == ERR_OK) {
              self.getData()
              this.$notify({
                title: '成功',
      ...
      getData() {
        ...
        getByDepartWeekly(params).then((res) => {
          if(res.code == ERR_OK) {
            ...
            this.commentRate = res.data.superComment
