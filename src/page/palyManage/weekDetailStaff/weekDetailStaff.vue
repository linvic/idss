<template>
  <div class='task-manage'>
    ...
    <div class="wrapper">
      ...
      <div class="title-header" style="border:1px solid rgba(0, 0, 0, 0.12);border-bottom:none;">
      ...
      <div class="circle-wrapper" style="border-bottom:1px solid rgba(0, 0, 0, 0.12);">
      ...
      <div class="special-title" style="border:1px solid rgba(0, 0, 0, 0.12);border-bottom:none;">
        <h4>一、任务列表</h4>
      </div>
      ...
      <div class="overHide">
        ...
        <el-table :data="tableDataCompany"
                  row-key="taskId"
                  class="company"
                  ...
                  <div class="">
                    ...
                    <div class="">
                      <span>重要等级：</span><select class="" name="" style='border:1px solid #8391a5;width:146px;height:30px;' @change='choseLeve($event)'>
                        ...
                        <option value="DEPARTMENT">部门重点</option>
                        <option value="PERSONALIMP">个人重点</option>
                        <option value="PERSONALORD" selected>个人普通</option>
                      </select>
                    </div>
                    <div class="" style="margin-top:10px;">
                      <span style="line-height:54px;float:left;">下周计划：</span><textarea name="name" rows="8" cols="80" style='padding: 5px 7px;border-radius: 4px;width:130px;height:44px;border:1px solid #8391a5'>{{props.row.taskWeeklyRelaUpdateEntity.weeklyPlan}}</textarea>
        ...
        <el-dialog title="完善任务" :visible.sync="dialogVisible" size="tiny" :before-close="handleClose" top='10%' class=" noticeManageModel">
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="112px" class="demo-ruleForm">
            ...
            <el-form-item label="执行人:" prop="value5" required style="margin-right: 40px !important;">
              <el-select v-model="ruleForm.value5" filterable placeholder="请选择执行人" style="width:100%;" @visible-change='goLink' disabled>
            ...
            <el-form-item label="任务类型:" prop="value4" required style="margin-right: 40px !important;">
              <el-select v-model="ruleForm.value4" placeholder="请选择类型" style="width:100%;">
            ...
            <div v-show="stretch">
              ...
              <el-form-item label="关联项目:" style="margin-right: 40px !important;">
                <el-select v-model="ruleForm.value7" placeholder="请选择关联项目" style="width:100%;">
      ...
      <div class="get-reward">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope='props'>
              <div class="">
                <el-select v-model="props.row.executorId" disabled filterable placeholder="请输入执行人" style="width:100%;" @change='goLink1(props.row.executorId,props.$index)'>
                  <el-option v-for="item in users" :key="item.userId" :label="item.userName" :value="item.userId">
                  </el-option>
                </el-select>
              </div>
      ...
      <div class="my-comment" v-if='weeklyDeptLevel=="LEVEL_TWO"'>
        <h3 comment-title>四、上级部门负责人点评</h3>
        <div class="comment-content">
          {{secondDeptHeaderComment}}
        </div>
      </div>
      <div class="my-comment">
        <h3 comment-title>{{taskTwo}}、总经理点评</h3>
...
<script type="text/ecmascript-6">
  // 注意标签和引入驼峰，可以有区别
  import Vue from 'vue'
  import { getByStaffWeekly,listByExecutor,publishDraftTasks,listAllUsers,listReportingUsers,departmentList,getProjectList,listTaskExecutors,userTaskReply,manageCommentStaff,manageWeeklyPlan,manageWeeklyComment,dragTask } from 'service/getData'
...
//      Tab
    ...
    methods: {
      ...
      submitForm5(formName1) {
        ...
        this.$refs[formName1].validate((valid) => {
          if (valid) {
            ...
            self.$refs['ruleForm5'].resetFields();
      ...
      sureNextPlan(){
        ...
        publishDraftTasks(params).then((res) => {
          if (res.code == ERR_OK) {
            this.$notify({
              title: '成功',
              message: '确认添加任务成功',
      ...
      listAllUsers1() {
        listTaskExecutors().then((res) => {
          if (res.code == ERR_OK) {
            ...
            this.tableData[index].executorId = this.weekUserId
  ...
  　　　　　　　document.body.scrollTop = distance
      ...
      getData() {
        ...
        getByStaffWeekly(params).then((res) => {
          if(res.code == ERR_OK) {
            ...
            this.commentRate = res.data.managerComment
            this.deptHeaderComment = res.data.deptHeaderComment
            this.secondDeptHeaderComment = res.data.secondDeptHeaderComment
            this.weeklyDeptLevel = res.data.weeklyDeptLevel
            this.weekUserId = res.data.userId
            if(res.data.weeklyDeptLevel=='LEVEL_TWO'){
              this.taskTwo = '五'
            }else{
              this.taskTwo = '四'
            }
      ...
      save(){
        ...
        if(this.commentRate==''||this.commentRate==null){
          ...
          manageCommentStaff(params).then((res) => {
            if(res.code == ERR_OK) {
              self.getData()
              this.$notify({
                title: '成功',
