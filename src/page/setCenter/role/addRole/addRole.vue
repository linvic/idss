  ...
  <div>
    ...
    <div class="wrapper">
      ...
      <div class="add-Role">
        ...
        <div class="role-content roles-temp">
             ...
             <ul class="first_children clear">
               <li v-for='(item,index) in result' class="first_role">
                 <label class="ant-checkbox-wrapper first_able" :class="item.resourceChilds.length>0 ? 'bord ':''">
                   <input type='checkbox' class='item-check-btn' :value="item.resourceId" :key="item.resourceId" v-model="item.hasAuth" v-on:click.stop="selectAll(item,index)">
                 ...
                 <ul class="second_children clear">
                   <li v-for="(itemChild,i) in item.resourceChilds">
                     <label class="ant-checkbox-wrapper second_able"  :class="itemChild.resourceChilds.length>0 ? 'bord ':''">
                       <input type='checkbox' class='item-check-btn' :value="itemChild.resourceId" :key="itemChild.resourceId" v-model="itemChild.hasAuth" v-on:click.stop="selectSecond(item,itemChild,i,index)">
                     ...
                     <ul class="clear"  :class="item.resourceChilds.length>0 ? 'bord ':''">
                       <li v-for="(itemChildLast,j) in itemChild.resourceChilds" class="lastLi" >
                         <label class="last_lab">
                           <input type='checkbox' class='item-check-btn' :value="itemChildLast.resourceId" :key="itemChildLast.resourceId" v-model="itemChildLast.hasAuth"  v-on:click.stop="selectAllThree(item,itemChild,itemChildLast,j,i,index)">
...
//  import Tab from 'components/common/tab'
  import { getRoleResource,addRole} from 'service/getData'
...
//      获取角色
      gerRole(param){
        var self=this;
        addRole({roleName:this.ruleForm.name,roleDesc:this.ruleForm.desc,enabled:this.ruleForm.resource,resourceIds:param}).then(function (res) {
          if(res.code == ERR_OK) {
            self.$notify({
              title: '成功',
              duration:3000,
