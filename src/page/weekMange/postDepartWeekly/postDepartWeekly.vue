  ...
  <div>
    ...
    <div class="wrapper">
      ...
      <div class="get-reward">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope='props'>
              <div class="">
                <el-select v-model="props.row.executorId" filterable placeholder="请输入执行人" style="width:100%;">
      ...
      <div class="my-comment" style="background:#fff;">
        <h3 comment-title>三、部门总结</h3>
        <div class="" style='margin-left:20px;font-size: 14px;line-height:24px;color: #333333;margin-top:15px;'>
          部门平均绩效评估分：{{averagePerformance}}
        </div>
        <div class="" style='margin-left:20px;font-size: 14px;line-height:24px;color: #333333;'>
          系统自动提交：<span style="color: #D93538;">{{autoSubUsers}}</span>
        </div>
        <textarea name="name" rows="8" cols="80" v-model='commentRate' style='width:1115px;border: 1px solid rgba(0,0,0,0.12);height:98px;padding:15px;margin:20px;'></textarea>
      </div>
        <div class="button-close"  @click='sureNextPlan'>
          提交
        </div>
        <div class="">
          <!--发布任务模态窗-->
          <el-dialog title="完善任务" :visible.sync="dialogVisible" size="tiny" :before-close="handleClose" top='10%' class=" noticeManageModel">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="112px" class="demo-ruleForm">
              ...
              <el-form-item label="执行人:" prop="value5" required style="margin-right: 40px !important;" @change='goLink'>
                <el-select v-model="ruleForm.value5" filterable placeholder="请选择执行人" style="width:100%;">
                  <el-option v-for="item in users" :key="item.userId" :label="item.userName" :value="item.userId">
                  </el-option>
                </el-select>
              </el-form-item>
...
<script type="text/ecmascript-6">
  // 注意标签和引入驼峰，可以有区别
  import { toPublishDeptWeekly,listReportingUsers,departmentList,morePersonalResults,impChangeOrdTask,listTaskExecutors,PublishDeptComment,listAllUsers,getUsersObj,getTaskType,getProjectList,allNextWeeklyPlan,userTaskReply,departReviewWeekly,dragTask} from 'service/getData'
...
//      Tab
  ...
  　　　　　　　document.body.scrollTop = distance
      ...
      sureNextPlan(){
        ...
        PublishDeptComment(params).then((res) => {
          if (res.code == ERR_OK) {
            this.$notify({
              title: '成功',
              message: '发起部门周报成功',
            ...
            //   type: 'success'
            // })
            this.$router.push({path: '/departWeeklyDetail', query:{id: res.data}})
          }else{
            this.$notify.error({
              title: '错误',
              message: res.msg
...
//        console.log(this.checkedValue)
      ...
      submitForm(formName) {
        let self = this
//         console.log(self.ruleForm.telObj)
        this.$refs[formName].validate((valid) => {
          if (valid) {
            var value71 = self.ruleForm.value71.join(',')
            self.ruleForm.date1 = new Date(self.ruleForm.telObj)
            if(self.ruleForm.date1==''){
              self.ruleForm.date1  = self.ruleForm.date1
            ...
            //self.tableData[self.index].repeatMode = self.ruleForm.value6
      ...
      completePerfect(val){
        ...
        if(this.tableData[index].planEndDate==''){
          this.tableData[index].planEndDate  = this.tableData[index].planEndDate
        ...
        this.getUsersObj1(this.tableData[index].executorId)
      ...
      getData() {
        ...
        toPublishDeptWeekly(params).then((res) => {
          if(res.code == ERR_OK) {
            ...
            this.commentRate = res.data.comment
