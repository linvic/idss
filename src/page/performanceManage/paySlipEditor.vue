    ...
    <div class="wrapper">
        ...
        <div class="idss-list">
            <div class="idss-list-top">
                <div class="idss-list-top-title">工资条列表
                    <span class="tip">{{salaryMonth}}</span>
                </div>
                <div class="idss-list-top-btn-groups">
                    <el-button size="small" type="primary" @click="linkSend" :disabled="!canSend">发送工资条</el-button>
                </div>
            </div>
            <el-table :data="tableData">
                ...
                <el-table-column label="打分确认状态">
                    <template slot-scope="scope">
                        <span v-if="scope.row.scoreStatus === 0" class="el-color-primary">待确认</span>
                        <span v-else-if="scope.row.scoreStatus === 1">已确认</span>
                        <span v-else>-</span>
                    </template>
                </el-table-column>
...
<script>
import {  listSalaryDetailPage,sendSalary,canSendSalary} from 'service/getData'
...
export default {
    ...
    methods: {
        ...
        linkSend() {
            this.$confirm('每个月的工资条只能发送一次，是否继续发送?', '发送工资条', {
