  ...
  <div>
    <div class="Department">
      <div class="head_title">
         <span class="left head_title_left">公司概览</span>
      </div>
      <ul class="module">
        <li class="module_children">
          <h2 class="top_title">
            <span class="title">公司任务总数</span>
          </h2>
          <div class="bm">
    ...
    <div class="near">
      <div class="near-header">
        <span class="near-newpage">近期周报</span>
        <el-button
          size="small"
          style='background:transparent;margin-right:30px;' class='setPlan complete' :disabled="!canManagerPlan" @click="tips()">制定公司计划</el-button>
        <span class="near-time">{{startDay}}  至  {{endDay}}</span>
      </div>
      <div>
        <ul>
          <li class="list-li"  v-for="item in taskAllWeeklyListVos">
            <span class="near-content-high" v-if="!item.managerReaded">{{item.deptName}}</span>
            <span class="near-content" v-if="item.managerReaded">{{item.deptName}}</span>
            <el-button
              size="small"
              style='background:transparent;' class="view" @click="look(item.deptWeeklyId)">查看</el-button>
          </li>
          <li v-if="!taskAllWeeklyListVos" class="noData">暂无数据</li>
        </ul>
      </div>
    </div>
  </div>
...
<script type="text/ecmascript-6">
  ...
  import {overview,overviewEchart,listRecentDeptWeekly} from '../../service/getData'
...
//          console.log(res);
      ...
      _listRecentDeptWeekly(){
        listRecentDeptWeekly().then((res)=>{
          if (res.code == ERR_OK) {
               this.canManagerPlan=res.data.canManagerPlan
               this.endDay=res.data.endDay
               this.startDay=res.data.startDay
               this.taskAllWeeklyListVos=res.data.taskAllWeeklyListVos
          }else{
            this.$notify.error({
              title: '提示',
              message: res.msg
